...
- A Let's Encrypt server with stapling on and no `resolver` in scope will also be reported by [ssl_stapling_without_resolver](ssl_stapling_without_resolver.md). Both findings are accurate; removing the stapling directives resolves both.
