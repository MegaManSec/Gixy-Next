...
- Point `resolver` at a local caching resolver (`127.0.0.1`, `[::1]`) or one inside your network, not at a public DNS service. A public resolver is reachable off-path by an attacker who can spoof replies, which is what [resolver_external](https://gixy.io/plugins/resolver_external/index.md) reports — at HIGH severity.
