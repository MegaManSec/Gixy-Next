...
1. **NGINX treats that as `NGX_LOG_EMERG`** and returns an error, so the configuration never loads. A cold start fails outright; a reload logs the error and silently keeps serving the *old* configuration, so a change you believe you deployed was never applied.
