...
1. A subsequent directive references a numeric capture group (`$1`, `$2`, ...) via `set $var ...`, `if ($var = ...)`, `if ($var != ...)`, or `if (-f ...)` (the `-d`, `-e`, `-x`, and negated file-test operators are treated identically).
...
- `$N` inside the pattern of a regex-match condition, e.g. `if ($var ~ "$1")`. The pattern is compiled once at configuration time, so `$1` there is literal regex text, not a runtime capture read.
- `$N` as the tested variable of an `if`, e.g. `if ($1)` or `if ($1 = ...)`. nginx compiles the tested operand to an indexed variable lookup rather than a complex value, and there is no variable named `1`, so such a configuration does not load at all: nginx rejects it with `[emerg] unknown "1" variable`.
